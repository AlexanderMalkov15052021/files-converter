...
export const getYaml = (json: JSON) => {
    ...
    // console.log(objJson.mLaneMarks); // 35163 35166
    ...
    const lanes = objJson.mRoads.map(
        (item: any, index: number) => {
            ...
            const index_1 = item.mLanes[0].mDirection === 2 ? indexes.end + shiftIndex : indexes.start + shiftIndex;
            const index_2 = item.mLanes[0].mDirection === 2 ? indexes.start + shiftIndex : indexes.end + shiftIndex;

            robotsBornPlaces.map((place: any, i: number) => {



                // const point_1_contents = targetLaneMarks[index_1].mark.contents;
                // const point_2_contents = targetLaneMarks[index_2].mark.contents;

                // const x_1 = point_1_contents.items[0].value;
                // const y_1 = point_1_contents.items[1].value;

                // const x_2 = point_2_contents.items[0].value;
                // const y_2 = point_2_contents.items[1].value;

                // console.log(index_1);

                const point_1 = objJson.mLaneMarks[index_1 - shiftIndex].mLaneMarkXYZW;
                const point_2 = objJson.mLaneMarks[index_2 - shiftIndex].mLaneMarkXYZW;
                ...
                if (nearbyWaysData[i].distance > distance || !nearbyWaysData[i].distance) {
                    nearbyWaysData[i].distance = distance;
                    nearbyWaysData[i].laneIndex = index;
                }
    ...
    const newlanes = lanes.map((lane: any, index: number) => {

        const data = nearbyWaysData.find(data => data.laneIndex === index);

        if (data) {

            const startPointIndex = lane.contents.items[0].value;
            const endPointIndex = lane.contents.items[1].value;

            const bidirectional = lane.contents.items[2].items[0].value.items[1].value;

            return [
                new YAML.Document(
                    [
                        startPointIndex,
                        data.markIndex + shiftIndex,
                        {
